...
class autoTrade :
    ...
    def buy_coin(self) :
        balance = upbit.get_balances() # 잔고 조회
        
        if balance > 5000 : # 잔고 5000원 이상일 때
            upbit.buy_market_order(self.ticker, balance * 0.9995)
            self.buy_yn = True

            buy_price = pyupbit.get_orderbook("KRW-XRP")['orderbook_units'][0]['ask_price'] # 최우선 매도 호가
    ...
    def sell_coin(self) :
        ...
        sell_price = pyupbit.get_orderbook("KRW-XRP")['orderbook_units'][0]['bid_price'] # 최우선 매수 호가
...
if __name__ == '__main__':
    ...
    slackBot = slack(SLACK_APP_TOKEN, SLACK_CHANNEL)

    start_cash = upbit.get_balances()
    ticker = "KRW-XRP"
